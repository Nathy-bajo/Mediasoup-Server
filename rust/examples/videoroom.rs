...
mod participant {
    use crate::participant::messages::{InternalMessage, ServerMessage, TransportOptions};
    use crate::room::Room;
        ...
        pub async fn started(&mut self, ctx: &mut WebSocketConnection) {
            ...
            self.attached_handlers.push(self.room.on_producer_add({
                ...
                move |participant_id, producer| {
                    ...
                    sender
                        .try_send(AllMessages::ServerMessage(ServerMessage::ProducerAdded {
            ...
            self.attached_handlers.push(self.room.on_producer_remove({
                ...
                move |participant_id, producer_id| {
                    ...
                    sender
                        .try_send(AllMessages::ServerMessage(ServerMessage::ProducerRemoved {
                            participant_id: *participant_id,
...
pub async fn ws_index(
    ...
    println!("roomsreg {:?}", rooms_registry);
    ...
    tokio::spawn(async move {
        while let Some(Ok(Message::Text(message))) =
            futures_lite::StreamExt::next(&mut wsc_clone).await
        {
...
pub struct TideState {
    room_registry: std::sync::Arc<RoomsRegistry>,
    worker_manager: std::sync::Arc<WorkerManager>,
}
use tide::Request;
...
async fn main() -> std::io::Result<()> {
    env_logger::init();
    let state = TideState {
        room_registry: std::sync::Arc::new(RoomsRegistry::default()),
        worker_manager: std::sync::Arc::new(WorkerManager::new()),
    };

    let mut app = tide::with_state(std::sync::Arc::new(state));

    app.at("/video").get(WebSocket::new(
        |req: Request<std::sync::Arc<TideState>>, wsc: WebSocketConnection| async move {
